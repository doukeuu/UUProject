...
#import "UUImageClippingController.h"

#define SCALE_FRAME_Y 100.0f
#define BOUNDCE_DURATION 0.3f

@interface UUImageClippingController ()
...
@implementation UUImageClippingController
...
- (void)initControlBtn {
    UIView *botmView=[[UIView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - 50.0f, [UIScreen mainScreen].bounds.size.width, 50)];
    botmView.backgroundColor=[UIColor blackColor];
    botmView.alpha=0.6;
    ...
    [botmView addSubview:confirmBtn];
}

- (void)cancel:(id)sender {
    if (self.clippingCompletion) {
        self.clippingCompletion(nil);
    }
}

- (void)confirm:(id)sender {
    if (self.clippingCompletion) {
        self.clippingCompletion([self getSubImage]);
    }
}
