...
#import "UULocationHandler.h"

@interface UULocationHandler () <CLLocationManagerDelegate>
...
@implementation UULocationHandler

+ (instancetype)shareHandler {
    static UULocationHandler *handler;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        handler = [[UULocationHandler alloc] init];
    });
    return handler;
}
...
+ (BOOL)locationEnabledAndPrompted:(BOOL)prompted {
    ...
    if (status == kCLAuthorizationStatusNotDetermined) {
        [[UULocationHandler shareHandler].manager requestWhenInUseAuthorization];
...
+ (void)location:(LocationBlock)block {
    UULocationHandler *handler = [UULocationHandler shareHandler];
    handler.locationBlock = [block copy];
    [handler.manager startUpdatingLocation];
}
...
+ (void)reverseGeocode:(CLLocation *)location completed:(GeocodeBlock)block {
    if (!location) return;
    UULocationHandler *handler = [UULocationHandler shareHandler];
    [handler.geocoder reverseGeocodeLocation:location completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
        if (error) {
...
+ (void)geocodeAddress:(NSString *)address completed:(GeocodeBlock)block {
    ...
    UULocationHandler *handler = [UULocationHandler shareHandler];
    [handler.geocoder geocodeAddressString:address completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
        if (error) {
            if (block) block(nil, error);
        } else {
            CLPlacemark *placemark = [placemarks firstObject];
