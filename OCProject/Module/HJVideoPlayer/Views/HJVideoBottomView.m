...
- (void)adjustSubviesOnlyFull:(BOOL)onlyFull{
    ...
    self.playTimeLbl.frame = CGRectMake(width - (onlyFull?0:height) - kTimeLabelWidth - 20, 0, kTimeLabelWidth, height);
    
    self.totalDurationLbl.frame = CGRectMake(width - (onlyFull?0:height) - 30, 0, kTimeLabelWidth, height);
    
    self.bufferSlider.frame = CGRectMake(kTimeLabelWidth - 20, (height - kSliderHeight)/2.f, CGRectGetMinX(self.playTimeLbl.frame) - kTimeLabelWidth + 20, kSliderHeight);
}
...
//        [_bufferSlider setProgressTrackColor:COLOR_THEME];
